...
# Custom CSS for modern look with dark chat backgrounds
st.markdown("""
<style>
    .main-header {
        font-size: 3rem;
...
def main():
    ...
    with st.sidebar:
        ...
        st.metric("🤖 Chat Assistant", "✅ Ready" if st.session_state.get('chat_ready') else "❌ No API Key")
    ...
    # Main content tabs
    tab1, tab2, tab3, tab4 = st.tabs(["💬 Chat Assistant", "📚 Knowledge Base", "📥 Literature Manager", "📊 Analytics"])
...
def chat_interface():
    ...
    with col2:
        temperature = st.slider("🌡️ Creativity", 0.0, 1.0, 0.3, 0.1)
    with col3:
        max_context = st.slider("📄 Max Sources", 1, 20, 8)
    ...
    with col1:
        if st.button("📋 What papers do I have?"):
            process_chat_message("What papers do I have in my knowledge base?", temperature, max_context)
    
    with col2:
        if st.button("🔬 Main research topics?"):
            process_chat_message("What are the main research topics in my collection?", temperature, max_context)
    
    with col3:
        if st.button("✍️ Help with writing"):
            process_chat_message("Help me write an introduction about quantum information theory", temperature, max_context)
    
    # Chat input
    if prompt := st.chat_input("Ask about your research papers..."):
        process_chat_message(prompt, temperature, max_context)
